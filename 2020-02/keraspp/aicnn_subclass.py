...
class CNN(Model):
    def __init__(self, nb_classes, in_shape=None):
        ...
        self.conv2A = Conv2D(32, kernel_size=(3, 3), activation='relu',input_shape=in_shape)
        self.conv2B = Conv2D(64, (3, 3), activation='relu')
        self.poolA = MaxPooling2D(pool_size=(2, 2))
        self.dropoutA = Dropout(0.25)
        self.flattenA = Flatten()
        self.denseA = Dense(128, activation='relu')
        self.dropoutB = Dropout(0.5)
        self.denseB = Dense(nb_classes, activation='softmax', name='preds')
        self.compile(loss='categorical_crossentropy',
                      optimizer='adadelta', metrics=['accuracy'])

    def call(self, inputs):
        h = self.conv2A(inputs)
        h = self.conv2B(h)
        h = self.poolA(h)
        h = self.dropoutA(h)
        h = self.flattenA(h)
...
class Machine():
    ...
    def run(self, epochs=100, batch_size=128, verbose=1):
        ...
        model = self.model
        fig = self.fig
        history = self.fit_model(epochs=epochs,
